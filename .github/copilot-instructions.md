This repository (auto-recipes) ingests recipe web pages, uses a LLM to parse free text into a strict Pydantic recipe schema, deduplicates ingredients with local embeddings, and persists results locally (SQLite). Optional external sinks (Notion, Google Calendar) are supported but not required.
...
- Orchestration: `src/orchestrate/run.py` — builds pipeline: fetch -> extract text -> LLM parse -> embed-index dedupe -> persist (SQLite/local artifacts).
...
- Copy `.env.example` -> `.env` and set `GEMINI_API_KEY` and any optional integration keys. The CLI calls `dotenv.load_dotenv()` at import time so `.env` must exist before running `python -m src.cli`.
- Quick commands used by humans (examples that agents can suggest/run):
  ...
  - Ingest a single page: `python -m src.cli ingest https://example.com/recipe`
...
- External sink behavior (Notion/Calendar) is optional and guarded by runtime checks. When adding integrations, follow the project's lazy-import pattern so core parsing and tests remain runnable without those packages.
...
- Notion API — optional: if you enable the Notion sink, the code will attempt to read DB schema and perform queries/page creation. Prefer defensive coding: operations may raise network/permission errors and are often best-effort (some exceptions are logged and swallowed).
